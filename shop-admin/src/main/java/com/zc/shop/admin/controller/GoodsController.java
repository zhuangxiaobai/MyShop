...
public class GoodsController {
    ...
    @ResponseBody
    public CommonResult register(@RequestBody @ApiParam(value="添加商品对象") List<GoodsCreateParam> goodsCreateParamList, HttpServletRequest request) {
       ...
       //创建商品信息
        int count = goodsService.create(goodsCreateParamList,userId);
        if(count > 0) {
    ...
    @ResponseBody
    public CommonResult goodslistIndex(@RequestBody GoodsSelectGaiParam goodsSelectGaiParam) {

        return CommonResult.success(goodsService.goodslistIndex(goodsSelectGaiParam));
    ...
    @ResponseBody
    public CommonResult goodslistIndex(@RequestBody GoodsSellListParam goodsSelectParam,HttpServletRequest request) {

       //获取当前用户id
        Users user = (Users) request.getAttribute("user");
        Integer userId = user.getId();
        goodsSelectParam.setSuppliersId(userId);
